...
| **List Filtering, Search, and Ordering**      | `blog/views.py:23-27`                               | `PostViewSet` uses DRF's `SearchFilter` and `OrderingFilter` — the resulting `list_post` tool exposes `search` and `ordering` params so LLMs can narrow/sort results without fetching everything    |
