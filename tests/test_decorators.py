...
from djangorestframework_mcp.decorators import (
    MCPViewSetDecorator,
    mcp_tool,
    mcp_viewset,
)
...
class TestMCPViewSetDecorator(unittest.TestCase):
    ...
    def test_decorator_rejects_plain_viewset(self):
        ...
        self.assertIn(
            "@mcp_viewset can only be used on classes that inherit from GenericViewSet",
            error_msg,
        )
...
class TestMCPToolDecorator(unittest.TestCase):
    ...
    def test_mcp_tool_requires_mcp_viewset(self):
        ...
        class TestIndividualViewSet(viewsets.GenericViewSet):
            @mcp_tool(
                name="test_individual_action",
                title="Test Individual",
                description="Test individual tool registration",
            )
        ...
        self.assertEqual(
            len(test_tools),
            0,
            "@mcp_tool without @mcp_viewset should not register any tools",
        )
        ...
        self.assertEqual(
            ...
            1,
            "@mcp_tool with @mcp_viewset should register the tool",
        )
    ...
    def test_mcp_tool_accepts_both_decorator_orders(self):
        ...
        for method, expected_detail in [
            (custom_action_1, False),
            (custom_action_2, True),
        ]:
            self.assertTrue(hasattr(method, "_mcp_needs_registration"))
...
class TestCustomActionValidation(unittest.TestCase):
    ...
    def test_custom_action_missing_input_serializer_fails_without_decorator_params(
        self,
    ):
